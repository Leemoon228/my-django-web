...
// fetch('/static/searchHistory.json')
//   .then(response => {
//     if (!response.ok) {
...
var searchHistory = {
    ...
    [
        {string : "тп на ам", 
            counts : 1, date : new Date(2024, 6, 10)},
        {string : "mili mitsubachi", 
            counts : 3, date : new Date(2024, 6, 12)},
        {string : "mili rightfully", 
            counts : 5, date : new Date(2024, 6, 15)},
    ]
 };
...
form.addEventListener('submit', e=> {
    ...
    if(!searchValue){
        ...
        const i = searchHistory.table.findIndex(e => e.string === searchValue.toLowerCase())
        if (i > -1) {
            searchHistory.table[i].counts += 1;
            searchHistory.table[i].date = new Date(Date.now())
        }
        else{
            let temp = {string : `${searchValue.toLowerCase()}`, counts : 1, date : new Date(Date.now())}
            searchHistory.table.push(temp)
            searchHistory
        }
